...
export async function findAccountByPasswordResetCode(passwordResetCode: string): Promise<Account | undefined> {
  const query = "SELECT * FROM account WHERE reset_password_code = $1";
  const result = await pool.query(query, [passwordResetCode]);

  if (result.rows.length === 0) {
    console.log("No account found for password reset code:", passwordResetCode);
    return;
  }
